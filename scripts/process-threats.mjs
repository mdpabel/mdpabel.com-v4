...
async function draftThreatPage(evidence, analysis) {
  return callStructuredResponse({
    ...
    instructions: [
      ...
      'Title must be SEO-friendly, client-friendly, and issue-first. Prioritize the visible problem a site owner would search for, such as fake CAPTCHA, casino popup, spam redirect, suspicious plugin folder, hidden .sc-backup folder, or weird theme folders. Put WordPress in the title. Use technical terms only when they support the visible issue. Avoid titles that read like analyst notes or list too many technical components.',
      'Good titles sound like real search queries from hacked site owners, not internal malware labels.',
      'Prefer titles like "WordPress Fake CAPTCHA and Casino Popup Malware" or "WordPress Spam Redirect Malware with Hidden .sc-backup Folder".',
      'Do not start the title with technical terms like MU-plugin, eval backdoor, persistence cluster, restore path, or remote-management framework unless no clear visible issue exists.',
      'If screenshots show obvious user-facing symptoms, those symptoms should usually dominate the title.',
